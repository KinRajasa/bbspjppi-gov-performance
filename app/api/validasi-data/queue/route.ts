...
/**
 * GET /api/validasi-data/queue
 * Mengambil antrean validasi untuk Katim dengan penguncian tombol
 * berdasarkan status revisi.
 */
export async function GET(request: Request) {
  try {
    const session = getRequestSession(request);
    if (!session || session.role !== 'KATIM') return NextResponse.json({ success: false, message: 'Antrean ini hanya dapat diakses oleh KATIM.' }, { status: 403 });
    ...
    // Status yang boleh masuk antrean Katim
    const reviewableStatuses: SubmissionStatus[] = [
      SubmissionStatus.SUBMITTED_TO_KATIM,
      SubmissionStatus.MENUNGGU_VALIDASI_KATIM,
      SubmissionStatus.REVISION_REQUIRED_BY_KAPOKJA, // Revisi dari Kapokja masuk antrean Katim
    ];
    ...
    const assignedIndicatorIds = await getAssignedPerformanceIndicatorIds(session.id);
    const submissions = await prisma.performanceSubmission.findMany({
      where: {
        fiscalYearId: fiscalYear.id,
        indicatorId: { in: assignedIndicatorIds },
    ...
    console.error('[VALIDASI_DATA_QUEUE_ERROR]', error);
    return NextResponse.json(
      {
        success: false,
